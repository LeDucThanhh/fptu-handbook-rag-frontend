...
const mockUnresolvedQuestions = [
    {
        ...
        priority: 'high',
        status: 'unresolved',
        systemResponse: 'Xin lỗi, tôi không có thông tin chi tiết về quy trình đăng ký học bổng khuyến khích học tập. Bạn có thể liên hệ trực tiếp với Phòng Đào tạo để được hỗ trợ.',
        confidence: 0.3,
    ...
    },
    {
        ...
        systemResponse: 'Thông tin về thời gian nộp hồ sơ học bổng có thể đã được cập nhật. Tôi khuyên bạn nên kiểm tra thông báo mới nhất từ trường.',
        confidence: 0.4,
    ...
    },
    {
        ...
        priority: 'low',
        status: 'unresolved',
        systemResponse: 'Tôi không chắc chắn về việc đăng ký môn học qua điện thoại. Bạn có thể thử truy cập hệ thống online hoặc liên hệ IT Support.',
        confidence: 0.2,
    ...
    },
    {
        ...
        priority: 'high',
        status: 'resolved',
        systemResponse: 'Lịch thi cuối kỳ đã được cập nhật. Bạn có thể xem lịch thi mới nhất tại trang web của trường.',
        confidence: 0.8,
...
const statusConfig = {
    unresolved: { label: 'Chưa giải quyết', color: 'text-destructive', bg: 'bg-destructive/10' },
    pending_review: { label: 'Chờ xem xét', color: 'text-warning', bg: 'bg-warning/10' },
    resolved: { label: 'Đã giải quyết', color: 'text-success', bg: 'bg-success/10' }
};
        ...
        <div className="min-h-screen bg-background p-6">
            <div className="max-w-7xl mx-auto space-y-6">
                ...
                <div className="flex justify-between items-center">
                    <div>
                        <h1 className="text-3xl font-bold text-foreground">Unresolved Queue</h1>
                    ...
                    <Card>
                        <CardContent className="p-6">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-2xl font-bold">{questions.filter(q => q.status === 'unresolved').length}</p>
                    ...
                    <Card>
                        <CardContent className="p-6">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-2xl font-bold">{questions.filter(q => q.status === 'pending_review').length}</p>
                    ...
                    <Card>
                        <CardContent className="p-6">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-2xl font-bold">{questions.filter(q => q.status === 'resolved').length}</p>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                        ...
                        {filteredQuestions.map((question) => (
                            <Card
                                key={question.id}
                                className={`cursor-pointer transition-all hover:shadow-md ${selectedQuestion === question.id ? 'ring-2 ring-primary' : ''
                                    }`}
                            ...
                            >
                                <CardContent className="p-4">
                                    <div className="flex justify-between items-start mb-3">
                                        <div className="flex-1">
                                            <h3 className="font-medium text-sm mb-2 line-clamp-2">{question.question}</h3>
                                        ...
                                        <div className="flex flex-col gap-1">
                                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${statusConfig[question.status].bg} ${statusConfig[question.status].color}`}>
                                                {statusConfig[question.status].label}
                                            </span>
                                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${priorityConfig[question.priority].bg} ${priorityConfig[question.priority].color}`}>
                        ...
                        {selectedQuestion ? (
                                ...
                                return (
                                    <Card>
                                        <CardHeader>
                                            <div className="flex justify-between items-start">
                                                <div>
                                                    ...
                                                    <CardDescription>
                                                        {question.studentName} ({question.studentId}) • {question.timestamp}
                                                    </CardDescription>
                                                </div>
                                                <div className="flex gap-2">
                                                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${statusConfig[question.status].bg} ${statusConfig[question.status].color}`}>
                                        ...
                                        <CardContent className="space-y-4">
                                            {/* System Response */}
                                            <div>
                                                <h4 className="font-medium mb-2">Phản hồi của hệ thống:</h4>
                                            ...
                                            {/* Mentor Notes */}
                                            <div>
                                                <h4 className="font-medium mb-2">Ghi chú của mentor:</h4>
                                                <textarea
                                                    value={mentorNotes[question.id] || ''}
                                                    onChange={(e) => setMentorNotes({ ...mentorNotes, [question.id]: e.target.value })}
                                            ...
                                            <div className="flex gap-2 pt-4 border-t">
                                                <Button
                                                    size="sm"
                                                    onClick={() => handleStatusChange(question.id, 'resolved')}
                                                ...
                                                <Button
                                                    size="sm"
                                                    variant="outline"
                                                    onClick={() => handleStatusChange(question.id, 'pending_review')}
                                                >
                            ...
                            <Card>
                                <CardContent className="p-8 text-center">
                                    ...
                                    <p className="text-muted-foreground">Chọn một câu hỏi để xem chi tiết</p>
                                </CardContent>
                            </Card>
                        )}
                    </div>
